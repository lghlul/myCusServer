...
<div class="external">
    <div class="page-header">
        <ul class="breadcrumb">
            <li>
                <i class="iconfont">&#xe648;</i>
                <a href="javascript:void(0)">新增会员卡</a>
            </li>
        </ul>
    </div>
    ...
    <div class="menu-panel">
        ...
        <form class="form-horizontal" id="addcard_form">
...
<span>*</span>
                ...
                <%-- <input type="hidden" name="id" value="${memberCard.id}"> --%>
                    <input type="text" name="membername" value="" maxlength="100" class="form-control" placeholder="请输入会员卡名称">
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-1 control-label">
                ...
                <div class="col-sm-5">
                    <input type="text" name="proinfo" value="" maxlength="200" class="form-control" placeholder="请输入会员卡说明" >
                </div>
            </div>
             <div class="form-group">
                <label class="col-sm-1 control-label">
<span>*</span>
                ...
                <div class="col-sm-8">
                    <!-- button type="button" class="btn btn-default">健身会员卡</button>
                    <button type="button" class="btn btn-default">动感单车</button>
                    <button type="button" class="btn btn-default">CrossFit</button>
                    <button type="button" class="btn btn-default">LeFITYoga</button>
                    <button type="button" class="btn btn-default">预售专用</button>
                    <button type="button" class="btn btn-default">CrossFit减脂塑形</button> -->
                  <div style="display: inline-block">
                	<c:forEach items="${memberCardType }" var="items" varStatus="status">
						 <c:choose>
	                    	<c:when  test="${status.index==0}">
	 								<button id="add_type"class="btn btn-default btn-success cardtype" type="button" data-id="${items.basicvalue}">${items.title} </button>
	 								<input type="hidden" value="${items.basicvalue}" name="cardtype" id="txt_cardtype">
			       			</c:when>
				       		<c:otherwise>
				       				   <button id="add_type" class="btn btn-default cardtype" type="button" data-id="${items.basicvalue}">${items.title} </button>
				       		 </c:otherwise>
	       				 </c:choose>
                	</c:forEach>
				  </div>
...
<span>*</span>
                ...
                <div class="col-sm-5 type">
                   	<c:forEach items="${memberCardTimeLong }" var="items" varStatus="status">
                   	     <c:choose>
	                    	<c:when  test="${status.index==0}">
	 								<button id="add_time" class="btn btn-default btn-inverse timelong" type="button" data-id="${items.basicvalue}">${items.title} </button>
	 								<input type="hidden" value="${items.basicvalue}" name="timelong" id="txt_timelong">
			       			</c:when>
				       		<c:otherwise>
				       				   <button id="add_time" class="btn btn-default timelong" type="button" data-id="${items.basicvalue}">${items.title} </button>
				       		 </c:otherwise>
	       				 </c:choose>
...
<%--                       <c:forEach items="${memberCardTimeLong}" var="item" varStatus="status">
	                    	...
	                    	<c:when  test="${item.checked}">
	 								<button class="btn timelong" type="button" data-id="${item.basicvalue}">${item.title} </button>
...
<span>*</span>
                ...
                <div class="col-sm-5">
                    <input type="number" name="amountmoney" value=""  class="form-control" placeholder="请输入金额" >
                </div>
            </div>
...
<span>*</span>
                ...
                <div class="col-sm-5">
                    <div class="onoffswitch">
                        ...
                        <input type="hidden" value="1" name="isuse" id="isuse_value">
                    </div>
                </div>
            </div>
...
<span>*</span>
                ...
                <div class="col-sm-5">
                   <div class="img-box">
 					...
 					<input type="hidden" name="mcardimg" id="mainfigure">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    <div class="tr-main">
                        <textarea name="memberrights" id="pcservicedetail"  style="width:700px;height:300px;"></textarea>
                    </div>
                </div>
            </div>
            <div class="form-group component">
                <div class="col-sm-8 col-sm-offset-1 center">
                  ...
                  <!--   <a class="btn btn-success" >取消</a> -->
                    <a   href="<%=request.getContextPath()%>/user/memberCard/listPage" class="btn btn-success"  >取消</a>
                </div>
            </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                     ...
                     <a class="btn btn-info" href="<%=request.getContextPath() %>/user/memberCard/listPage">返回门店列表</a>
                        <a class="btn btn-success" href="<%=request.getContextPath() %>/memberCard/store/addPage">继续添加</a>
        ...
        <div class="modal fade" id="addcard_failure" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                ...
                <!--         <button type="button" class="btn btn-default" data-dismiss="modal">确定</button>
        		  ...
        		  <!--弹出内容-->
        <!-- 模态框（Modal）成功后的框！ -->
        <div class="modal fade" id="all_success" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
                    ...
                    <!-- 	<a id="a_infoLink" class="btn btn-default" href="">查看咨询</a> -->
                       <!--  <a class="btn btn-primary" href="javascript:void(0)">复制链接</a> -->
                        ...
                        <a class="btn btn-success" href="<%=request.getContextPath() %>/user/memberCard/addPage">继续添加</a>
                        <!-- <button type="button" class="btn btn-default" data-dismiss="modal">关闭 -->
                        </button>
                   <!--      <button type="button" class="btn btn-success">
                            提交更改
          ...
          <!-- 模态框（Modal）失败后的框！ -->
        <div class="modal fade" id="all_failure" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
...
<script src="<%=request.getContextPath()%>/assets/js/jquery.validate.js"></script>
    <script src="<%=request.getContextPath()%>/assets/js/membercard/add_membercard.js"></script>
