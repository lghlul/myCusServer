...
<html>
	...
	<head>
		...
		<link rel="stylesheet" href="<%=request.getContextPath()%>/assets/css/outside/style-my-lesson.css">
		<link rel="stylesheet" href="<%=request.getContextPath()%>/assets/css/outside/font_lqw37te7tmjnstt9.css"/>
						...
						<c:choose >
						  <c:when test="${course!=null && fn:length(course) > 0}">
							...
							<c:forEach items="${course}" var="item" varStatus="status">
								<div class="le-courses-box courses-box" data-id=${item.id } data-type=${item.type } >
