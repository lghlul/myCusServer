	...
	<body>
		<div id="app">
			<div class="body-box">
				...
				<%-- 		<c:choose>
						   <c:when test="${course.isexperience==1}">

						   		<div><span>课次*</span></div>
						   ...
						   <c:otherwise> --%>
								<div class="clearfix">
									...
									<div class="right clearfix">
									<span v-on:click="add($index)" :class="{off:item.count==100}" <c:if test="${course.isexperience==1}"> style="pointer-events: none" </c:if> ></span>
									<span id="totalclass">{{item.count}}</span>
									<span class="" v-on:click="reduce($index)" :class="{off:item.count==8}" <c:if test="${course.isexperience==1}"> style="pointer-events: none"</c:if> ></span></div>
								</div>
						<%--    </c:otherwise>
						</c:choose> --%>
        	...
        	<button class="mint-msgbox-btn mint-msgbox-cancel btn_div" style="">确定</button>
				...
				<script>
					$(function(){
						...
						//var isexperience="${course.isexperience}";
...
$(function(){
	//var isexperience="${course.isexperience}";
	//if(isexperience){
		/* $('.div_isexperience').attr(); */
		//$(".div_isexperience *").attr("disabled",true);
	//}
	 ...
	 var isexperience="${course.isexperience}";
 	 ...
 	 $('#sure_pay').click(function(){
        ...
        $.post(baseUrl+"/topayServlet?isexperience="+isexperience+"&order_id="+order_id+"&total_fee="+pay_gz+"&totalClass="+totalClass+"&openid="+openId+"&cid="+cid+'&type=1', function(data){
            	  ...
            	  WeixinJSBridge.invoke('getBrandWCPayRequest',{
            	       "appId" : appId,"timeStamp" : timeStamp, "nonceStr" : nonceStr, "package" : packageValue,"signType" : "MD5", "paySign" : paySign
            	                ...
            	                if(res.err_msg == "get_brand_wcpay_request:ok"){
            	                    //alert("支付成功");
            	                	   window.location.href=baseUrl+"/outside/buyCard/buyPriCoachSuccess";
