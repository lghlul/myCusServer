$(function(){
	...
	function  getcourseGroup(){
	    ...
	    $.ajax({
		      type:"GET",
		      url:baseUrl+'/outside/buyCard/courseGrouplist?offset='+offset+'&&limit=9',
		    	  ...
		    	  if(data.resultCode==200){
			    	  if(data.records !=null){
			    		  ...
			    		  for(var i=0;i<recordlist.length;i++){
			    			...
			    			html="开营日期"+startDate+"<br>";
			    			//循环精品团课排期
			    			for ( var j = 0; j < timelist.length; j++) {
			    				html+="周"+timelist[j].concatweek.replace(",", '、')+"&nbsp;&nbsp;"+timelist[j].starttime+"~"+timelist[j].endtime+"&nbsp;&nbsp;";
							}
			    			 $('#appendgroupDiv').append(
									'<a href="'+baseUrl+'/outside/buyCard/courseGroupDetail?startDate='+startDate+'&endDate='+endDate+'&id='+recordlist[i].courseid+'&storeid='+recordlist[i].storeid+'">'+
													...
													'<div class="main">'+
														...
														'<div class="training-center-item-title2">'+
															...
															'<div class="training-center-item-subTitle2">'+
																''+recordlist[i].storename+' / '+recordlist[i].coachnames+' /'+html+' '+
															'</div>'+
														'</div>'+
													'</div>'+
												'</div>'+
