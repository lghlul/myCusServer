...
/*
	...
	public PageResponse selectAllByList(Page page,  EmployeeBuyCard employeeBuyCard) throws Exception {
		...
		//判断会员状态是否为空
		if (employeeBuyCard.getStatus()!=null) {
			contion.andStatusEqualTo(employeeBuyCard.getStatus());
		}
		//判断会员来源是否为空
		if (employeeBuyCard.getMembersource()!=null) {
			contion.andMembersourceEqualTo(employeeBuyCard.getMembersource());
		}
		//判断会员类型是否为空
		if (employeeBuyCard.getMembertype()!=null) {
			contion.andMembertypeEqualTo(employeeBuyCard.getMembertype());
		}
		//判断门店不为空
		if (employeeBuyCard.getStoreid()!=null) {
			contion.andStoreidEqualTo(employeeBuyCard.getStoreid());
		}
		//总数
		int count = employeeBuyCardDao.countByExample(example);
		//分页条件
		example.setLimitStart(Integer.valueOf(page.getOffset()));
		example.setLimitEnd(Integer.valueOf(page.getLimit()));
		example.setOrderByClause("addtime desc");

		List<EmployeeBuyCard> membersList=employeeBuyCardDao.selectByExample(example);
	...
	*//**
	...
	public void  selectMemberByMobile(String mobile,Integer mcardid,Integer id) throws Exception{
		...
		//查询会员是否已经拥有
		List<EmployeeBuyCard> member=employeeBuyCardDao.selectByExample(example);
...
*/
	...
	public EmployeeBuyCard  selectMemberById(Integer id) throws Exception {
		 return employeeBuyCardDao.selectByPrimaryKey(id);
	}
	...
	public void editSave(EmployeeBuyCard member) throws Exception {
		...
		EmployeeBuyCard beforeMember=employeeBuyCardDao.selectByPrimaryKey(member.getId());
		if(beforeMember ==null){
			throw new Exception("没有该数据或数据已经被删除，无法修改");
		}
		//查询该号码是否已经添加过，如果添加过，则抛出异常
		...
		employeeBuyCardDao.updateByPrimaryKeySelective(member);
	...
	public void cancleBind(Employee employee) throws Exception {
		EmployeeBuyCard employeeBuyCard = new EmployeeBuyCard();
		employeeBuyCard.setUserid(employee.getEmployeeid());
		employeeBuyCard.setPhonenumber(employee.getMobilephone());
		employeeBuyCardDao.updateByUserId(employeeBuyCard);
		employeeDao.updateByPrimaryKeySelective(employee);
	}
...
//	}
	...
	public int delete(Integer id) {
		return employeeBuyCardDao.delete(id + "");
	}
}
