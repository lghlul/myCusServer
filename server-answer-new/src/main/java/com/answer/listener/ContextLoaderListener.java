...
public class ContextLoaderListener implements ServletContextListener{
	...
	private static final org.slf4j.Logger logger = LoggerFactory.getLogger(ContextLoaderListener.class);
		...
		Runnable r = new Runnable() {
			...
			public void run() {
				while(true){
					...
					if(roomList != null){
						//logger.info("task...roomList=" + JSON.toJSONString(roomList));
						for(Room room : roomList){
							//logger.info("task...room=" + JSON.toJSONString(room));
							List<RoomQuestion> answerList = roomQuestionMapper.selectAnswerByRoomID(room.getRoomID());
							//logger.info("task...answerList=" + JSON.toJSONString(answerList));
							//创建人答对数量
							...
							if(answerList != null && answerList.size() > 0){
								...
								if(isFinish){
									User user = new User();
									logger.info("task...createRightNum=" + createRightNum + ",rightNum=" + rightNum);
									...
									logger.info("task...r=" + JSON.toJSONString(r));
