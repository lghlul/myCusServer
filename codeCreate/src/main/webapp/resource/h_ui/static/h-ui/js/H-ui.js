...
(function($) {
	...
	ColorPicker.prototype.setValue = function(color, notSetInput) {
		...
		if (options.updateBackground) {
			$(options.updateBackground).css('background-color', "red");
		}
			...
			that.find(options.titCell).on(options.trigger,function(){
				if ($(this).next().is(":visible")) {
					if (options.type == 2) {
						...
						$(this).next().slideUp(options.speed).end().parent().removeClass(options.className);
						if ($(this).find("b")) {
							$(this).find("b").html("+");
						}
					}
					...
					if (options.type == 3) {
						$(this).next().slideDown(options.speed).end().parent().addClass(options.className);
						if ($(this).find("b")) {
							$(this).find("b").html("-");
						}
					} else {
						that.find(options.mainCell).slideUp(options.speed);
						that.find(options.titCell).parent().removeClass(options.className);
