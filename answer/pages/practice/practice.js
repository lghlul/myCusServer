    ...
    putAnswer: function () {
        ...
        /*that.data.answerList.forEach(function (chooseItem, chooseIndex) {
            ...
            if (answerIndex > -1) {
                ...
                console.log(that.data.questionData.question.answerList[answerIndex].choose);
            }
    ...
    getMistake: function (id, callback) {
        ...
        wx.request({
            ...
            success: function (result) {
                ...
                if (result.data.resultCode == 0) {
                    result.data.resultData.question.answerList.forEach(function (item, index) {
                        let wrongIndex = result.data.resultData.wrongAnswerID.split(',').findIndex(ele => ele == item.ansID);
                        if (wrongIndex > -1) {
                            item.choose = 'choose';
                        }
